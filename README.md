...
- **外部アプリ再生**: スマホ・タブレットで Infuse / VLC に渡して再生(ブラウザ再生と並存)
...
| 外部プレイヤー | スマホ・タブレットの「アプリで開く」で使うアプリ。Infuse(既定)/ VLC を切替(Android は VLC 固定) |
...
- **iOS / iPadOS では HLS モードを使う**: WebKit は WebCodecs で MPEG-2 を復号できないため、ブラウザ内での TS 直モードは使えません。iPhone / iPad で Mirakurun を選ぶと「TS 直接再生に対応していません」と案内が出るので、**「アプリで開く」から Infuse / VLC で再生**するか、EPGStation の HLS ソースをご利用ください
...
- **外部アプリ再生**: スマホ・タブレットではプレイヤー下に「アプリで開く」を表示。Mirakurun は TS 直 URL を、EPGStation は m2ts 変換 URL を Infuse / VLC に渡す。デスクトップでは非表示
